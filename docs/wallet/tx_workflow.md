...
3. Use the below JavaScript code to send Epic Cash to other wallets.

    In the line number 125, update value of the `dest` key with the receiver wallet's ngrok address.

    In the line number 114, multiply the amount of Epic Cash you want to send with 10^9 and update value of the `amount` key with it. For example, we are sending `0.01` Epic Cash here, but the value of the `amount` key is `10000000`.
